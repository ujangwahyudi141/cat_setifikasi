$(document).ready(function(){ // Ketika halaman sudah siap (sudah selesai di load)
  ...
  $("#provinsi").change(function(){ // Ketika user mengganti atau memilih data provinsi
    ...
    $.ajax({
      ...
      url: "require/option_kota.php", // Isi dengan url/path file php yang dituju
...
$(document).ready(function(){ // Ketika halaman sudah siap (sudah selesai di load)
  ...
  $("#provinsi2").change(function(){ // Ketika user mengganti atau memilih data provinsi
    ...
    $.ajax({
      type: "POST", // Method pengiriman data bisa dengan GET atau POST
      url: "require/option_kota2.php", // Isi dengan url/path file php yang dituju
      data: {provinsi2 : $("#provinsi2").val()}, // data yang akan dikirim ke file yang dituju
      dataType: "json",
      beforeSend: function(e) {
        if(e && e.overrideMimeType) {
          e.overrideMimeType("application/json;charset=UTF-8");
      ...
      success: function(response){ // Ketika proses pengiriman berhasil
        ...
        // set isi dari combobox kota
        // lalu munculkan kembali combobox kotanya
        $("#kota2").html(response.data_kota).show();
      },
      error: function (xhr, ajaxOptions, thrownError) { // Ketika ada error
        alert(thrownError); // Munculkan alert error
      }
...
$(document).ready(function(){ // Ketika halaman sudah siap (sudah selesai di load)
  ...
  $("#jenjang").change(function(){ // Ketika user mengganti atau memilih data provinsi
    $("#jurusan").hide(); // Sembunyikan dulu combobox kota nya
    $("#loading3").show(); // Tampilkan loadingnya
  
    $.ajax({
      type: "POST", // Method pengiriman data bisa dengan GET atau POST
      url: "require/option_jurusan.php", // Isi dengan url/path file php yang dituju
      data: {jenjang : $("#jenjang").val()}, // data yang akan dikirim ke file yang dituju
      dataType: "json",
      beforeSend: function(e) {
        if(e && e.overrideMimeType) {
          e.overrideMimeType("application/json;charset=UTF-8");
      ...
      success: function(response){ // Ketika proses pengiriman berhasil
        ...
        // set isi dari combobox kota
        // lalu munculkan kembali combobox kotanya
        $("#jurusan").html(response.data_pendidikan).show();
      },
      error: function (xhr, ajaxOptions, thrownError) { // Ketika ada error
        alert(thrownError); // Munculkan alert error
      }
